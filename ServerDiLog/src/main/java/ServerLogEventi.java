...
import org.w3c.dom.*;
import org.xml.sax.*;
import it.unipi.dii.inginf.vGiannini.adprg.EventoDiNavigazione;
        ...
        try( ServerSocket server = new ServerSocket(PORTA) ){
             while(true){
                 try(Socket socket = server.accept();
                     ...
                     Class<?>[] classes = new Class[] { EventoDiNavigazione.class };
                     XStream xstream = new XStream();
                     XStream.setupDefaultSecurity(xstream);
                     xstream.allowTypes(classes);
                    EventoDiNavigazione evento = (EventoDiNavigazione)(xstream).fromXML(inputStream.readUTF()); // (00)
